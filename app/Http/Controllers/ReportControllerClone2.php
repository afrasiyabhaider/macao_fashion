            ...
            $query = TransactionSellLine::join(
                ...
                ->select(
                    ...
                    DB::raw("(SELECT SUM(TSL.quantity - TSL.quantity_returned) FROM transactions 
                        JOIN transaction_sell_lines AS TSL ON transactions.id=TSL.transaction_id
                        WHERE transactions.status='final' AND transactions.type='sell' $location_filter 
                        AND TSL.product_refference = p.refference AND TSL.updated_at > now() - INTERVAL 7 day) as seven_day_sold"),
                    DB::raw("(SELECT SUM(TSL.quantity - TSL.quantity_returned) FROM transactions 
                        JOIN transaction_sell_lines AS TSL ON transactions.id=TSL.transaction_id
                        WHERE transactions.status='final' AND transactions.type='sell' $location_filter 
                        AND TSL.product_refference = p.refference AND TSL.updated_at > now() - INTERVAL 15 day) as fifteen_day_sold"),
    ...
    public function getstockInOutReport(Request $request)
    {
            ...
            if ($request->ajax()) {
                ...
                $products = $query->select(
                    ...
                    // DB::raw("(SELECT SUM(ltd2.quantity) FROM location_transfer_details 
                    // JOIN location_transfer_details as ltd2 ON variations.id = ltd2.variation_id)
                    //         WHERE transactions.status='final' AND transactions.type='sell' $location_id 
                    //         AND TSL.variation_id=variations.id) as total_transfered"),  
                    DB::raw("(SELECT SUM(TSL.quantity - TSL.quantity_returned) FROM transactions 
                            JOIN transaction_sell_lines AS TSL ON transactions.id=TSL.transaction_id
                            WHERE transactions.status='final' AND transactions.type='sell' $location_filter 
    ...
    public function getstockInOutGroupedReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $products = $query->select(
                ...
                // DB::raw("SUM(CASE WHEN vld.location_id  = $location_id AND (DATE(vld.created_at) BETWEEN '$from_date' AND '$to_date') THEN vld.qty_available ELSE 0 END) as stock"),
                // DB::raw("SUM(CASE WHEN ltd2.transfered_from = $location_id AND (DATE(transfered_on) BETWEEN '$from_date' AND '$to_date') THEN ltd2.quantity ELSE 0 END) as total_transfered"),
                // DB::raw("SUM(CASE WHEN ltd2.location_id = $location_id AND (DATE(transfered_on) BETWEEN '$from_date' AND '$to_date') THEN ltd2.quantity ELSE 0 END) as transfered_added"),
            //     DB::raw("
            //     (SELECT SUM(CASE 
            //         WHEN ltd2.transfered_from = $location_id 
            //             AND (DATE(ltd2.transfered_on) BETWEEN '$from_date' AND '$to_date') 
            //         THEN ltd2.quantity 
            //         ELSE 0 
            //     END) as total_transfered 
            //     FROM location_transfer_details AS ltd2 
            //     JOIN variations AS v ON ltd2.variation_id = v.id
            //     ) as subquery_alias
            // "),
            // DB::raw("(SELECT SUM(CASE 
            //         WHEN ltd2.location_id = $location_id 
            //             AND (DATE(ltd2.transfered_on) BETWEEN '$from_date' AND '$to_date') 
            //         THEN ltd2.quantity 
            //         ELSE 0 
            //     END) as transfered_added 
            //     FROM location_transfer_details AS ltd2 
            //     JOIN variations AS v ON ltd2.variation_id = v.id
            //     )"),    
                DB::raw("SUM(CASE WHEN ltd2.location_id = $location_id AND (DATE(transfered_on) BETWEEN '$from_date' AND '$to_date') THEN ltd2.quantity ELSE 0 END) as total_transfered"),            
            ...
            return DataTables::of($products)
                ...
                // ])
                ->removeColumn('id')
                ->rawColumns(['mass_delete', 'printing_qty', 'unit_price', 'total_transfered','refference','total_refference', 'location_name', 'total_sold','front_total_sold', 'total_adjusted', 'stock', 'actions', 'image', 'detail'])
                ->make(true);
        }

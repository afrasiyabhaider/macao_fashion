...
class UpdateProductThatHaveSameName extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'product:update-name';
     ...
     * @var string
     */
    protected $description = 'Replace product names that contain same name with unique and meaningful names';
    ...
    public function handle()
    {
        ...
        DB::beginTransaction();
        try {
            // Get products where the name contains duplicates
            $duplicateNames = Product::select('name')
                ->groupBy('name')
                ->havingRaw('COUNT(*) > 1')
                ->pluck('name');

            if ($duplicateNames->isEmpty()) {
                $this->info('No products found with the same name.');
                return;
            }

            $totalProducts = Product::whereIn('name', $duplicateNames)->count();
            $this->info("Total products with same name: $totalProducts");
            ...
            // Process products in chunks to avoid memory exhaustion
            Product::whereIn('name', $duplicateNames)->chunk(100, function ($products) use ($progressBar) {
